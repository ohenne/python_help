...
for it in range(47,107,tint):
  ...
  n,x,_ = ax[0].hist(con_in.ravel(), bins, histtype='step',alpha=0.5,density=True,label=timestart.strftime("%H:%M")+"-"+timestartplus.strftime("%H:%M"), color=colors[((it-47)/6)+1])
...
fig2.savefig('plots/convergence_pdf_pureconv.pdf') #'testplots/'+EXPID+'_age_vr_all_otherrange.pdf')
