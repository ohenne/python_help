...
EXPID ='lindp2K_500tr_neuconv1' #'test1plus4K' #'lind_p2K' #'test1plus4K'# 'lindp2K'#
DIR = '$modelo2/UCLA_lind/p2K_large/lind_p2K_budget_conv.nc'
dir = '/nbi/ac/conv1/henneb/results/coldpool/'
data_in    = Dataset('/nbi/ac/cond4/rawdata/UCLA_lind/p2K_large/lind_p2K_budget_pure_conv.nc' ,'r') #'/lind_p2K/lind_p2K_budget_conv.nc', 'r')
...
con_in = np.multiply(con_in,1000)
...
#  con_in = np.multiply(con_in,1000)
  ...
  ax2[0].plot([mean00,mean00], [0, 0.010], 'k-', lw=2,color=colors[((it-47)/5)+1])
  mean00 = np.mean(con_in_mask)
  ax2[1].plot([mean00,mean00], [0,0.010], 'k-', lw=2,color=colors[((it-47)/5)+1])
  ...
  ax2[1].plot([median00,median00], [.150, .160], 'k-', lw=2,color=colors[((it-47)/5)+1])

  timestart=timestart+timedelta(minutes=60)

left = 0.02 #, width = .05, .5
bottom = 0.025 #, height = .45, .0
right = 0.98 #left + width
top = 0.97 # bottom + height
...
ax2[1].text(left, top, 'b) grid points with tracers',
        horizontalalignment='left',
        verticalalignment='top',
        transform=ax2[1].transAxes)

ax2[0].set_xlim(-15,20)
ax2[0].legend()
ax2[0].set_xticks([])
ax2[0].set_yticks([0,0.1,0.2])
ax2[0].set_yticklabels(['.0','.1','.2'])
#ax2[0,0].legend(loc='upper left')

ax2[1].set_xlim(-15,20)
ax2[1].set_xticks([-10,0.0,10])
ax2[1].set_xticklabels([-10,0,10])
ax2[1].set_ylim(0,0.16)
ax2[1].set_yticks([0,0.04,0.08,0.12,0.16])
ax2[1].set_yticklabels(['.00','.04','.08','.12','.16'])
#ax2[1,0].legend()
#ax2[1,0].legend(loc='upper left')
ax2[1].set_xlabel('convergence / 10$\mathregular{^{-3}}$ s$\mathregular{^{-1}}$')
ax2[1].set_ylabel('                                         normalized frequency / 10$\mathregular{^{3}}$ s')
fig2.savefig('plots/convergence_pdf_pureconv_newunits.pdf') #'testplots/'+EXPID+'_age_vr_all_otherrange.pdf')
